...
async function execute(args, flags) {
  ...
  const result = await executeWithOptionalAudit({
    command: 'write', spreadsheetId, range, newValue,
    execute: async () => {
      ...
      if (sheetName.toLowerCase() === 'inventory') {
        ...
        await sheets.spreadsheets.batchUpdate({
          spreadsheetId,
          requestBody: { requests: [{ repeatCell: { range: { sheetId, startRowIndex: grid.startRowIndex, endRowIndex: grid.endRowIndex }, cell: { userEnteredFormat: { backgroundColor: color } }, fields: 'userEnteredFormat.backgroundColor' } }] },
        });
        await updateStatus(sheets, spreadsheetId, sheetName, rowNumber, wasEmpty ? 'New' : 'Modified');
      }
  ...
  if (spreadsheetId === CLEANING.SPREADSHEET_ID && range) {
    await formatCleaningDateCell(sheets, spreadsheetId, range, oldValue_, newValue);
  }
  return result;
}
