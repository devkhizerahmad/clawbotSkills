...
function normalizeLeaseWithStatus(lease) {
  return {
    tenantName: lease.tenantName || "",
    email: lease.email || "",
...
async function listLeases({ command }) {
  const allLeases = await fetchAllLeases();

  if (command === "signed-leases") {
    const leases = allLeases.filter(
      (lease) => pickLeaseStatus(lease) === "signed"
    );
    const normalizedLeases = leases.map(normalizeLease);
  ...
  if (command === "unsigned-leases") {
    const leases = allLeases.filter(
      (lease) => pickLeaseStatus(lease) === "unsigned"
    );
    const normalizedLeases = leases.map(normalizeLease);
