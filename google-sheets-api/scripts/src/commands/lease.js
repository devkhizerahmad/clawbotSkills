...
const {
  ...
  addEmail,
  logAllEmails,
} = require("../services/email/sentEmailRegistry");
...
async function lease({ sheets, args, flags, command }) {
  ...
  await logAudit({
    ...
    cell: "N/A",
    oldValue: "N/A",
    newValue: `Starting lease command with details: ${leaseStr.substring(
      0,
      100
    )}...`,
    source: "LEASE_CMD",
  ...
  // Prorate
  const prorateMatch = leaseStr.match(
    /prorate(?:d)?\s*(?:amount)?\s*\$?(\d+)/i
  );
  ...
  await logAudit({
    ...
    cell: "N/A",
    oldValue: "Raw Text",
    newValue: `Tenant: ${tenantName}, Apartment: ${apartment}, Room: ${
      room || "N/A"
    }, Rent: $${amount}, Prorate: $${prorate}, Period: ${startDate} to ${endDate}, Contact: ${
      contact || "N/A"
    }, Email: ${email || "N/A"}`,
    source: "LEASE_CMD",
  });
  ...
  const leaseContractId = await saveLeaseContract({
    tenantName,
    email,
    pdfPath,
    propertyAddress: apartment,
  ...
  if (email) {
    ...
    if (hasEmail(email)) {
      console.log(
        `[EmailRegistry] Email already exists, skipping send: ${email}`
      );
      ...
      await logAudit({
        ...
        sheet: "Lease_Email",
        cell: "N/A",
        oldValue: "Duplicate Email Attempt",
        newValue: `Skipped sending duplicate lease agreement to ${email} for ${tenantName}`,
        source: "LEASE_CMD",
      });
    } else {
      ...
      } else if (emailResult?.reason === "already-sent") {
        await logAudit({
          user: auditUser,
          sheet: "Lease_Email",
          cell: "N/A",
          oldValue: "Email Already Sent",
          newValue: `Skipped sending duplicate lease agreement to ${email} for ${tenantName}`,
          source: "LEASE_CMD",
        });
      }
  ...
  await logAudit({
    ...
    newValue: `Lease operation completed successfully for ${tenantName} - Apartment: ${apartment}, Room: ${
      room || "N/A"
    }, Rent: $${amount}, PDF: ${pdfPath}`,
    source: "LEASE_CMD",
  });
