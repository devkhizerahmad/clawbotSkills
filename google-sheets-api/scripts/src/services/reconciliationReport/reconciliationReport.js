...
function toRecord(row) {
  ...
  Object.entries(row).forEach(([key, value]) => {
    const mapped = HEADER_LOOKUP[normalizeHeader(key)];
    if (mapped) {
      record[mapped] = value;
    }
  });
...
function rowToRecord(row, headerMap) {
  ...
  HEADERS.forEach((header, index) => {
    const sourceIndex = headerMap ? headerMap[header] : index;
    record[header] = sourceIndex != null ? row[sourceIndex] : "";
  });
  return record;
}
...
function buildHeaderMap(firstRow) {
  ...
  normalizedHeaders.forEach((cell, index) => {
    const mapped = HEADER_LOOKUP[cell];
    if (mapped && map[mapped] == null) {
      map[mapped] = index;
    }
  });

  const hasStatusAndTenant = map.Status != null && map["Tenant Name"] != null;
  return hasStatusAndTenant ? map : null;
}

function normalizeStatus(value, difference) {
  const status = String(value || "")
    .trim()
    .toUpperCase();
  if (status === "MATCH" || status === "MISMATCH" || status === "MISSING")
    return status;
  if (difference == null) return "MISMATCH";
  return Math.abs(difference) < 0.005 ? "MATCH" : "MISMATCH";
}

function normalizeReconciliationData(data) {
  if (!Array.isArray(data)) return [];

  const firstArrayRow = data.find((item) => Array.isArray(item));
  const detectedHeaderMap = buildHeaderMap(firstArrayRow);

  return data
    .map((row) => {
      if (detectedHeaderMap && Array.isArray(row) && row === firstArrayRow) {
        return null;
      }

      const record = rowToRecord(row, detectedHeaderMap);
      if (!record) return null;

      const expected = parseMoney(record["Expected Rent"]);
      const actual = parseMoney(record["Actual Amount"]);
      const providedDiff = parseMoney(record.Difference);
      const computedDiff =
        expected != null && actual != null ? actual - expected : null;
      const difference = providedDiff != null ? providedDiff : computedDiff;

      const normalized = [
        normalizeStatus(record.Status, difference),
        String(record["Tenant Name"] || "").trim(),
        String(record["Pays As"] || "").trim(),
        String(record.Email || "").trim(),
        String(record.Phone || "").trim(),
        String(record.Apartment || "").trim(),
        String(record.Room || "").trim(),
        record["Expected Rent"] == null || record["Expected Rent"] === ""
          ? ""
          : expected != null
          ? formatMoney(expected)
          : String(record["Expected Rent"]).trim(),
        record["Actual Amount"] == null || record["Actual Amount"] === ""
          ? ""
          : actual != null
          ? formatMoney(actual)
          : String(record["Actual Amount"]).trim(),
        difference != null
          ? formatMoney(difference)
          : String(record.Difference || "").trim(),
      ];
...
async function generateReconciliationReport(data, outputPath) {
  const rows = normalizeReconciliationData(data);
  ...
  pdf.setFontSize(10);
  pdf.text(`Total Records: ${rows.length}`, margin + 5, yPos + 8);
  ...
  rows.forEach((row, rowIndex) => {
    ...
    row.forEach((cell, i) => {
      ...
      if (i === 0) {
        ...
        else if (text === "MISSING") pdf.setTextColor(...COLORS.muted);
...
if (require.main === module) {
  ...
  generateReconciliationReport(sampleData)
    .then((result) =>
      console.log(`PDF successfully generated at: ${result.outputPath}`)
    )
