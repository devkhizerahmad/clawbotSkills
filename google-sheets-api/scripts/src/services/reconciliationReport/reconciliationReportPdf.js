...
async function generateReconciliationReport(data, outputPath) {
  ...
  // Audit log for successful PDF generation (mutation complete)
  try {
    const stats = data.reduce(
      (acc, row) => {
        if (row[0] === 'MATCH') acc.matched++;
    ...
    await logAudit({
      ...
      cell: 'N/A',
      oldValue: 'No report',
      newValue: `Report generated: ${stats.matched} matches, ${stats.mismatched} mismatches out of ${data.length} records`,
      source: 'RECONCILIATION_PDF',
    });
  } catch (err) {
