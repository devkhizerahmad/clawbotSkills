...
- `read`, `write`, `append`, `clear`, `batchGet`, `batchWrite`
- `highlight`, `unhighlight`, `Add apartment`
- `lease`, `generate_rent_reconciliation_report`, `get_rent_reconciliation_report`, `get_unpaid_rent_report`
...
## Rent Reconciliation Report

The `generate_rent_reconciliation_report` command fetches data from a sheet, generates a PDF report comparing expected vs. actual rent, and emails it as an attachment.

### Usage

```bash
# Generate latest report and email it
npm run generate_rent_reconciliation_report

# Generate report for a specific date
npm run generate_rent_reconciliation_report -- --date=2026-03-06

# Specify spreadsheet, range, and output path
npm run generate_rent_reconciliation_report -- <spreadsheetId> "'My Sheet'!A:J" "./reports/report.pdf"
```

### Arguments

- **spreadsheetId** (optional): The ID of the spreadsheet. Defaults to the one in `config.js`.
- **range** (optional): The sheet and range in A1 notation. Defaults to `'Rent Reconciliation'!A:J`.
- **outputPath** (optional): The path to save the generated PDF. Defaults to the project root.
- **--date**: Filters the report to include only records matching the specified date (e.g., `YYYY-MM-DD`).

---

## MongoDB Rent Reconciliation Report

The `get_rent_reconciliation_report` command retrieves rent reconciliation records from a MongoDB collection for a specified month.

### Prerequisites

Ensure your `.env` file contains a valid `MONGO_URI` connection string.

### Usage

```bash
# Get report for a specific month (YYYY-MM) via npm script
npm run get_rent_reconciliation_report -- 2026-03

# Get only unpaid/missing tenants for a specific month
npm run get_rent_reconciliation_report -- 2026-03 unpaid

# Or using the main CLI script
node scripts/sheets-cli.js get_rent_reconciliation_report 2026-03
```

### Arguments

- **month** (required): The month to retrieve records for, in `YYYY-MM` format.
- **status** (optional): Filter by status. Can be `unpaid` or `missing` to show tenants with an unpaid or missing payment status.

---

## Unpaid/Missing Rent Report

The `get_unpaid_rent_report` command is a dedicated command to retrieve only tenants with an "unpaid" or "missing" payment status for a given month. It is a convenient wrapper around `get_rent_reconciliation_report`.

### Usage

```bash
# Get unpaid/missing tenants for a specific month
npm run get_unpaid_rent_report -- 2026-03
```
