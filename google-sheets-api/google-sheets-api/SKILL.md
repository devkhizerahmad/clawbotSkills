...
description: OpenClaw skill that installs a Google Sheets CLI with setup steps and commands for read/write, batch, formatting, sheet management, and **comprehensive audit logging for all operations**.
---
...
Provide a production-ready Google Sheets CLI for OpenClaw. This skill supports data reads/writes, batch operations, formatting, and sheet management with service account authentication and **comprehensive audit logging for all operations**.
...
- **You require complete audit trails of all spreadsheet changes.**

## Not a fit

- You must use end-user OAuth consent flows (this skill is service-account focused).
- You only need lightweight, one-off edits.
- **You don't need audit logging of operations.**

## Comprehensive Audit Logging

**All Google Sheets commands now automatically record changes to an Audit_Log spreadsheet.** This provides a complete history of all operations performed through the CLI.

### Key Features:
- **Automatic logging** for all 27+ commands
- **Value change detection** - only logs when actual changes occur
- **Detailed context** including timestamps, user, sheet, cell, old/new values
- **Batch operation support** - aggregates multiple changes into single entries
- **Read-only operation logging** - tracks information retrieval
- **No-op optimization** - skips logging when no changes occur

### Audit Log Location:
- Spreadsheet ID: `1RobrLNYSmMUyq53dUcdmj2ePaU2YkagqLqgIgx7M4OU`
- Sheet: `Audit_Log`
- Format: Timestamp | User | Sheet | Cell | Old Value | New Value | Source

### Commands That Create Audit Logs:

**Data Modification Commands:**
- `write`, `append`, `clear`, `batchWrite`
- `highlight`, `unhighlight`
- `allUpdatesCleaning`, `formatCleaningDateCell` - **(FIXED)** Now properly logs bulk date operations
- `create`, `addSheet`, `deleteSheet`, `renameSheet`, `copyFormat`
- `format`, `merge`, `unmerge`, `borders`
- `resize`, `autoResize`, `freeze`
- `batch`, `lease`, `addApartment`

**Read-Only Commands (Informational):**
- `read`, `batchGet`, `getFormat`, `info`
...
- Move-Out Verification: Before changing a date in Column X, the Assistant MUST ask the user: "Is this for a move-out?".
- If Yes: Use the --moveout flag (e.g., write ... --moveout). The date cell will be formatted with a Blue background.
- If No: Run the command without the flag. The date cell will be formatted with a Yellow background.
- Date Column: Column X (previously W) is monitored for cleaning date changes.
- Color Formatting: The cell color is automatically applied based on the move-out status (Blue for move-outs, Yellow for others).
