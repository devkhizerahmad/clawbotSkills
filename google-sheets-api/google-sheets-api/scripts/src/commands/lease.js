...
async function lease({ sheets, args, flags, command }) {
  ...
  // Log audit entry for PDF generation
  await logAudit({
    user: 'ASSISTANT',
    sheet: 'Agreement',
    cell: 'PDF',
    oldValue: 'N/A',
    newValue: `Agreement PDF generated at ${pdfPath}`,
    source: 'LEASE_CMD',
  });
  ...
  if (email) {
    ...
    // Log audit entry for email sending
    await logAudit({
      user: 'ASSISTANT',
      sheet: 'Agreement',
      cell: 'Email',
      oldValue: 'N/A',
      newValue: `Agreement emailed to ${email}`,
      source: 'LEASE_CMD',
    });
  } else {
  ...
  // Old value fetch karne ka logic
  const oldDataResp = await sheets.spreadsheets.values.get({
    spreadsheetId,
    range: `${sheetName}!D${rowIndex}:W${rowIndex}`,
  });
  const oldValue = oldDataResp.data.values?.[0].join(', ') || 'N/A';
  ...
  try {
    ...
    if (cleaningRowIndex !== -1 && room) {
      ...
      if (roomNum >= 1 && roomNum <= 3) {
        ...
        const oldCleaningResp = await sheets.spreadsheets.values.get({
          spreadsheetId,
          range: `${cleaningSheetName}!${tCol}${cleaningRowIndex}:${cCol}${cleaningRowIndex}`,
        });
        const oldCleaningValue = oldCleaningResp.data.values?.[0].join(', ') || 'N/A';
        await sheets.spreadsheets.values.batchUpdate({
          spreadsheetId,
          requestBody: {
            valueInputOption: 'USER_ENTERED',
        ...
        await logAudit({
          ...
          oldValue: oldCleaningValue,
          newValue: `Tenant ${tenantName}, Contact ${contact || email || ''}`,
          source: 'LEASE_CMD',
        });
      }
    }
  ...
  try {
    ...
    if (rentRowIndex !== -1) {
      ...
      // Old value fetch
      const oldRentResp = await sheets.spreadsheets.values.get({
        spreadsheetId,
        range: `${rentSheetName}!D${rentRowIndex}:H${rentRowIndex}`,
      });
      const oldRentValue = oldRentResp.data.values?.[0].join(', ') || 'N/A';
      ...
      await logAudit({
        ...
        oldValue: oldRentValue,
        newValue: `Tenant ${tenantName}, Rent $${amount}, Email ${email || ''}, Contact ${contact || ''}`,
        source: 'LEASE_CMD',
      });
    }
  ...
  try {
    ...
    if (invDataRowIndex !== -1) {
      ...
      // Old value fetch
      const oldInvDataResp = await sheets.spreadsheets.values.get({
        spreadsheetId,
        range: `${invDataSheetName}!D${invDataRowIndex}:E${invDataRowIndex}`,
      });
      const oldInvDataValue = oldInvDataResp.data.values?.[0].join(', ') || 'N/A';
      ...
      await logAudit({
        ...
        oldValue: oldInvDataValue,
        newValue: `Tenant ${tenantName}, Rent $${amount}`,
        source: 'LEASE_CMD',
      });
