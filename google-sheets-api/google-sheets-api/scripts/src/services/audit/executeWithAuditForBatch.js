...
async function executeWithAuditForBatch({
  ...
  // 🔹 Important: Use for-of loop for async-safe logging
  for (const req of requests) {
    // 🔹 Structural operation: addSheet, deleteSheet, etc.
    if (req.addSheet) {
      await logAudit({
        user: 'ASSISTANT',
        sheet: req.addSheet.properties?.title || '',
        cell: 'N/A',
        oldValue: 'N/A',
        newValue: 'Sheet Created',
        source: command || 'SYSTEM',
      });
      continue; // skip updateCells logic for this req
    }

    if (!req.updateCells) continue;
